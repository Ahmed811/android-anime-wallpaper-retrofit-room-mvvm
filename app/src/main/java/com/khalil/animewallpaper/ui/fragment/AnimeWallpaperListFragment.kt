...
import com.khalil.animewallpaper.util.Constant.Companion.QUERY_PAGE_SIZE
import com.khalil.animewallpaper.util.Resource
import okhttp3.internal.addHeaderLenient

class AnimeWallpaperListFragment : Fragment() {
     lateinit var _binding:FragmentAnimeWallpaperListBinding
    lateinit var viewModel: WallpaperViewModel
    lateinit var animeAdapter: AnimeAdapter
    val TAG="AnimeWallpaper"
    override fun onCreateView(
        inflater: LayoutInflater,
        container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View? {
        _binding=FragmentAnimeWallpaperListBinding.inflate(inflater,container,false)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        animeAdapter.setOnItemClickListener {
            ...
            findNavController().navigate(R.id.action_animeWallpaperListFragment_to_wallpaperViewFragment,bundle)
        }
        viewModel.animeWallpaper.observe(viewLifecycleOwner, Observer {response->
            when(response){
                is Resource.Success->{
                    hideProgressBar()
                        hideErrorCard()
...
fun showErrorCard(){
   _binding.cvError. visibility=View.VISIBLE
    _binding.btnRetry.setOnClickListener {
        viewModel.getAnimeWallpaper(20)
        hideErrorCard()
    }
}
    fun hideErrorCard(){
   _binding.cvError. visibility=View.INVISIBLE
    ...
    _binding.rcAnimeList.apply {
        adapter=animeAdapter
        layoutManager=GridLayoutManager(requireContext(),2)
        setHasFixedSize(true)
        addOnScrollListener(this@AnimeWallpaperListFragment.scrollListener)
    }
